          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="space-y-6">
               <div className="flex items-center justify-between mb-2 px-1">
                  <div>
                    <h2 className="text-2xl font-black text-primary tracking-tight">Select Delivery Address</h2>
                    <p className="text-xs text-secondary font-medium italic">Choose a saved location or add a new one.</p>
                  </div>
                  <Button 
                    onClick={handleLocationAutoFill}
                    variant="secondary"
                    className="flex items-center gap-2 px-5 py-3 text-[10px] group bg-white border border-border shadow-sm hover:border-accent"
                  >
                    {isLocating ? (
                 ...
                 {savedAddresses.length > 0 ? savedAddresses.map((addr) => (
                    ...
                    <div className="md:col-span-2 py-8 text-center text-secondary border border-dashed border-border rounded-[24px]">
                 ...
                 <motion.button
                   ...
                   className={`
                      ...
                      ${showNewForm ? 'border-accent bg-accent/5' : 'border-slate-200 hover:border-accent/40 bg-white shadow-sm'}
                   `}
                 >
                    <div className={`p-4 rounded-full ${showNewForm ? 'bg-accent text-white' : 'bg-slate-100 text-secondary'}`}>
                       <Plus size={24} />
                    </div>
                    <span className="text-xs font-black uppercase tracking-widest text-primary">Add New Address</span>
                 </motion.button>
               </div>
            </div>
            ...
            <AnimatePresence>
              {showNewForm && (
                <motion.div 
                  ...
                  className="bg-white rounded-[40px] p-8 md:p-12 border border-border shadow-premium overflow-hidden"
                >
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h3 className="text-xl font-black text-primary tracking-tight">New Shipping Detail</h3>
                        <p className="text-[10px] text-secondary font-black uppercase tracking-widest">Enter specific delivery info</p>
                      </div>
                    </div>
                    <button 
                      onClick={() => setIsMapOpen(true)}
                      className="flex items-center gap-2 px-6 py-3 bg-slate-50 border border-border rounded-2xl text-[10px] font-black uppercase tracking-widest text-primary hover:bg-slate-100 transition-colors"
                    >
                  ...
                  <form className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="md:col-span-2 space-y-2 group">
                      ...
                      <input 
                        ...
                        className="w-full px-8 py-5 bg-slate-50 rounded-[24px] border-2 border-transparent font-bold text-primary focus:bg-white focus:border-accent/40 transition-all outline-none"
                      />
                    ...
                    <div className="space-y-2 group">
                      ...
                      <div className="relative">
                        ...
                        <input 
                          ...
                          className="w-full pl-16 pr-8 py-5 bg-slate-50 rounded-[24px] border-2 border-transparent font-bold text-primary focus:bg-white focus:border-accent/40 transition-all outline-none"
                        />
                      </div>
                    ...
                    <div className="md:col-span-2 space-y-2 group relative">
                      ...
                      <textarea 
                        ...
                        className="w-full px-8 py-5 bg-slate-50 rounded-[24px] border-2 border-transparent font-bold text-primary focus:bg-white focus:border-accent/40 transition-all outline-none resize-none"
                      ></textarea>
                      ...
                      <AnimatePresence>
                        {showSuggestions && addressSuggestions.length > 0 && (
                          <motion.ul 
                            ...
                            animate={{ opacity: 1, y: 0 }}
                            exit={{ opacity: 0, y: -10 }}
                            className="absolute z-20 top-[90px] left-0 right-0 mt-2 bg-white rounded-2xl border border-border shadow-premium max-h-48 overflow-y-auto"
                          >
                            {addressSuggestions.map((sug, idx) => (
                              <li 
                                ...
                                className="px-6 py-4 border-b border-gray-50 last:border-0 hover:bg-slate-50 cursor-pointer transition-colors"
                              >
                                <span className="text-sm font-bold text-primary">{sug.label}</span>
                              </li>
                            ))}
                          </motion.ul>
                        )}
                      ...
                      <div className="flex flex-wrap gap-4">
                        {['Home', 'Work', 'Other'].map(type => (
                          <button
                            ...
                            className={`px-6 py-2.5 rounded-2xl text-[11px] uppercase tracking-wider font-bold transition-all border-2 ${formData.type === type ? 'border-accent bg-accent/10 text-accent ring-2 ring-accent/20' : 'border-slate-200 text-secondary hover:border-slate-300 bg-white shadow-sm'}`}
                          >
                            {type}
                          </button>
                        ))}
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-center gap-4 p-8 bg-white rounded-[32px] border border-border shadow-sm">
                <div className="p-3 bg-green-50 rounded-xl">
                  <Truck className="text-green-500" size={24} />
                </div>
                <div>
                  <p className="text-[10px] font-black uppercase tracking-widest text-primary">Free Shipping Guaranteed</p>
                  <p className="text-xs text-secondary font-medium">Mobixa Priority Logistics ensures safe, express delivery.</p>
                </div>
              </div>
            </div>
          </div>
